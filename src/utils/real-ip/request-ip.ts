...
export class RequestIp extends InteractsWithHeaders {
    /**
    ...
    getFromForwardedFor(): string | undefined {
        /// ingress nginx
        if (this.hasIpInHeader('x-original-forwarded-for')) {
            return this.ipInHeader('x-original-forwarded-for')
        }
    ...
    ipInHeader(name: string): string | undefined {
        let value = this.header(name);
        if (!value)
            return value;

        return this.findIp(value.split(','))
    }
    ...
    fromRaw(): string | undefined {
        if (this.hasRaw()) {
            return new RequestIp(this.request.raw).getClientIp()
        }
    }
    ...
    isIpv4(value?: string): boolean {
        const regex = /^(?:(?:\d|[1-9]\d|1\d{2}|2[0-4]\d|25[0-5])\.){3}(?:\d|[1-9]\d|1\d{2}|2[0-4]\d|25[0-5])$/;
    ...
    isIpv6(value?: string): boolean {
        const regex = /^((?=.*::)(?!.*::.+::)(::)?([\dA-F]{1,4}:(:|\b)|){5}|([\dA-F]{1,4}:){6})((([\dA-F]{1,4}((?!\3)::|:\b|$))|(?!\2\3)){2}|(((2[0-4]|1\d|[1-9])?\d|25[0-5])\.?\b){4})$/i;
        return (value != null && regex.test(value));
    }
    ...
    debug() {
        ...
        if (this.hasHeaders()) {
            potentialHeaders.forEach(name => {
                if (this.hasIpInHeader(name))
                    ips[name] = this.ipInHeader(name);
                else
                    ips[name] = null;
            });
        }
